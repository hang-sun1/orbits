...
impl KeplerBody {
    pub(crate) fn new(name: &str, params: KeplerParams, mass: f64, central_mass: f64, time: JulianDate) -> Self {
        let (pos, vel) = KeplerBody::kepler_to_state_vectors(params);
        KeplerBody {
            name: name.to_string(),
            pos,
            vel,
    ...
    fn kepler_to_state_vectors(elements: KeplerParams) -> (Vector3, Vector3) {
        let e = elements.eccentricity;
        let a = elements.semimajor_axis; // * 1.49597870691e11;
        let mu = 1.32712440042e20;
        ...
        // let nu = 2.0 * (((1.0+e)/(1.0-e)).sqrt() * (E/2.0).tan()).atan();
        let r = a * (1.0 - e * E.cos());

        let o = r*Vector3(nu.cos(), nu.sin(), 0.0);
        let odot =  ((mu*a).sqrt()/r) * Vector3(-1.0*E.sin(), (1.0-e*e).sqrt() * E.cos(), 0.0);
        // let rx = r * (Omega.cos()*(omega+nu).cos() - Omega.sin()*(omega+nu).sin()*i.cos());
        // let ry = r * (Omega.sin()*(omega+nu).cos() + Omega.cos()*(omega+nu).sin()*i.cos());
        // let rz = r * (i.sin()*(omega+nu).sin());
        ...
        let r3d = (1.0 /1.49597870691e11) * Vector3(rx, ry, rz);
        ...
        let rdot = (86400.0 / 1.49597870691e11) * Vector3(rdotx, rdoty, rdotz);
...
impl SolarObject for KeplerBody {
    ...
    fn tick(&mut self, delta_t: f64) {
        ...
        // let mut stepper = Dopri5::new(self.clone(), self.time.0, self.time.0 + delta_t, inputs, 1.0e-2);
        let r = stepper.integrate().unwrap();
...
impl ode_solvers::System<State> for KeplerBody {
    fn system(&self, _x: f64, y: &State, dy: &mut State) {
        ...
        dy[1] = y[4];
        dy[2] = y[5];
        dy[3] = -self.central_mass * GRAV * y[0] / r.powi(3);
        dy[4] = -self.central_mass * GRAV * y[1] / r.powi(3);
        dy[5] = -self.central_mass * GRAV * y[2] / r.powi(3);
    }
}
